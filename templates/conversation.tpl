<div class="boheme stream-entry is-friend">
   ...
   <aside class="options">
    ...
    <div class="meta">
     ...
     {{if $item.lock}}<span title="{{$item.lock}}" class="locked ion-ios-locked-outline">-  </span>{{/if}}
   ...
   <div class="entry-details">
    ...
    <aside class="feedback">
      {{if $item.star}}<span class="ion-ios-star-outline mark" title="{{$item.star.toggle|escape:'html'}}"></span>{{/if}}
      {{if $item.vote.share}}<span class="ion-ios-redo-outline share" title="{{$item.vote.share.0|escape:'html'}}"></span>{{/if}}
      <span class="ion-ios-heart-outline heart" title="{{$item.vote.like.0|escape:'html'}}" data-hearts=""></span>
    </aside>
